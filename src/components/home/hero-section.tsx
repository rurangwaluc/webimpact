    ...
    <section className="w-full overflow-x-hidden px-3 pb-7 pt-3 sm:px-5 lg:pb-10">
      ...
      <div className="mx-auto max-w-7xl">
        <div className="relative overflow-hidden rounded-[1.6rem] border border-black/10 bg-white shadow-2xl shadow-black/10 dark:border-white/10 dark:bg-[#0b0b0b] sm:rounded-[2rem]">
          <div className="pointer-events-none absolute right-[-90px] top-[-90px] h-72 w-72 rounded-full bg-[#fd5b38]/20 blur-3xl" />
          ...
          <div className="relative grid gap-8 px-4 py-6 sm:px-7 sm:py-10 lg:min-h-[calc(100svh-105px)] lg:grid-cols-[0.92fr_1.08fr] lg:items-center lg:px-12 lg:py-12 xl:px-14">
            <div className="max-w-2xl">
              ...
              <h1
                className="hero-fade-up mt-5 max-w-3xl text-[clamp(2.05rem,10vw,3.2rem)] font-bold leading-[0.98] tracking-[-0.058em] text-black dark:text-white sm:text-[clamp(2.6rem,6vw,4.35rem)] lg:text-[clamp(2.75rem,5vw,4.5rem)]"
            ...
            <div className="mx-auto w-full max-w-[640px] lg:mr-0">
              <div
                ...
                style={{ animationDelay: "220ms" }}
              >
                <div className="relative h-[260px] overflow-hidden sm:absolute sm:inset-0 sm:h-full">
                  <img
                  ...
                  <div className="absolute left-3 top-3 z-10 max-w-[calc(100%-4.5rem)] rounded-full border border-white/15 bg-black/35 px-3 py-1.5 text-[9px] font-bold uppercase tracking-[0.1em] text-white shadow-xl backdrop-blur-md sm:left-5 sm:top-5 sm:px-4 sm:py-2 sm:text-xs">
                    <span className="line-clamp-1">
                      Business systems that create control
                    </span>
                  </div>

                  <div className="absolute right-3 top-3 z-10 grid h-9 w-9 place-items-center rounded-xl border border-white/15 bg-black/35 text-white shadow-xl backdrop-blur-md sm:right-5 sm:top-5 sm:h-11 sm:w-11 sm:rounded-2xl">
                    <Cpu className="h-4 w-4 sm:h-5 sm:w-5" />
                  </div>
                </div>

                <div className="relative z-10 p-3 sm:absolute sm:bottom-5 sm:left-5 sm:right-5 sm:p-0">
                  <div className="hero-float rounded-[1.35rem] border border-white/15 bg-black/78 p-3 text-white shadow-2xl shadow-black/35 backdrop-blur-xl sm:rounded-[1.75rem] sm:bg-black/58 sm:p-5">
              ...
              <div className="mt-3 grid grid-cols-3 gap-2 sm:hidden">
                <MobileProof label="Trust" icon={<Globe2 className="h-4 w-4" />} />
                <MobileProof
                  label="Control"
                  active
                  icon={<Layers3 className="h-4 w-4" />}
                />
                <MobileProof label="Growth" icon={<BarChart3 className="h-4 w-4" />} />
              </div>
            </div>
          </div>
    ...
    <div
      className={
        active
          ...
          : "rounded-2xl border border-black/10 bg-black/[0.03] p-3 text-black dark:border-white/10 dark:bg-white/[0.06] dark:text-white"
      }
    >
      <div className={active ? "text-white" : "text-[#fd5b38]"}>{icon}</div>
      <p className="mt-3 text-[10px] font-bold opacity-75">{label}</p>
    </div>
  );
}