...
import { ArrowRight, ExternalLink, Loader2, LockKeyhole } from "lucide-react";
...
function getAdminRedirectUrl() {
  ...
  return `${origin.replace(/\/$/, "")}/auth/callback?next=/admin`;
}
...
export default function AdminLoginPage() {
  ...
  async function handleLogin(e: React.FormEvent<HTMLFormElement>) {
    ...
    if (error) {
      ...
      setStatus("success");
      setMessage(
        "Check your email for the secure login link. After opening it, you will be sent directly to the admin panel.",
      );
    }
  }
    ...
    <main className="min-h-screen bg-white px-4 py-10 dark:bg-[#070707] sm:px-6 lg:px-8">
      <div className="mx-auto flex min-h-[calc(100vh-5rem)] max-w-7xl items-center justify-center">
        <div className="grid w-full max-w-5xl overflow-hidden rounded-[2.75rem] border border-black/10 bg-[#f7f7f7] shadow-2xl dark:border-white/10 dark:bg-[#111111] lg:grid-cols-[0.9fr_1.1fr]">
          <div className="hidden border-r border-black/10 p-10 dark:border-white/10 lg:block">
            <div className="relative">
              <Link
                href="/"
                className="text-sm font-black uppercase tracking-[0.22em] text-[#fd5b38]"
              >
                WebImpact Lab
              </Link>
            ...
            <div className="mx-auto max-w-md">
              ...
              <form onSubmit={handleLogin} className="mt-8 grid gap-4">
                ...
                <button
                  ...
                  className="inline-flex items-center justify-center gap-2 rounded-full bg-[#fd5b38] px-6 py-4 text-sm font-black text-white shadow-lg shadow-[#fd5b38]/25 transition hover:-translate-y-0.5 hover:bg-[#e84a2b] disabled:cursor-not-allowed disabled:opacity-60"
                >
                  {isSubmitting ? (
                    <>
              ...
              <div className="mt-6 flex flex-wrap items-center gap-4 text-sm font-black">
                <Link
                  href="/admin"
                  className="inline-flex items-center gap-2 text-[#fd5b38] hover:text-[#e84a2b]"
                >
                  Open admin panel <ExternalLink className="h-4 w-4" />
                </Link>
                <Link
                  href="/"
                  className="inline-flex text-black hover:text-[#fd5b38] dark:text-white"
                >
                  Back to website
                </Link>
              </div>
